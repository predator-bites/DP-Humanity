...
from .rabby_file import Rabby, RabbyImp


class Humanity:
    def __init__(self, private_key):
        self.rabbyimp = RabbyImp(private_key=private_key)
        self.rabby = Rabby()
    ...
    async def go_connect_claim(self, context: BrowserContext):
        try:
            ...
            rabbymask_button = page.get_by_test_id(test_id="rk-wallet-option-metaMask")
            await expect(rabbymask_button).to_be_enabled()
            await rabbymask_button.click()
            
            await self.rabby.connect(context=context)
            await self.rabby.add_network(context=context)

            verify_button = page.get_by_test_id(test_id="rk-auth-message-button")
            await expect(verify_button).to_be_enabled()
            await verify_button.click()

            await self.rabby.confirm(context=context)
